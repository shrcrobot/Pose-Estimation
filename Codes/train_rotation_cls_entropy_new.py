...
model_save_dir = os.path.join(BASE_DIR, "models", "rotation_cls_entropy_box_new_"+datetime.datetime.now().strftime('%d_%H:%M:%S'))
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        #loss = F.nll_loss(cls_phi, to_cls(angle[:,2]))
        loss.backward()
...
filename= "rotation_cls_entropy_box_new_"+datetime.datetime.now().strftime('%d_%H:%M:%S')+".txt"

def writeToFileOne(epoch):
    fl = open(os.path.join(log_save_dir, filename),'a+')
    if epoch == 0:
        fl.write(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+"\n")
        fl.write("train_loss\t eval_loss\t psi\t theta \t phi \t total \n")
        return
    # fl.write("epoch: " + str(epoch)+"\n")
    for i in range(len(train_losses)):
        fl.write(train_losses[i]+ "\t" + eval_losses[i]+ "\t" + psi_acc[i]+ "\t" + theta_acc[i]+ "\t" + phi_acc[i]+ "\t" + total_angle_acc[i] + "\n")
    train_losses.clear()
    eval_losses.clear()
    psi_acc.clear()
    theta_acc.clear()
