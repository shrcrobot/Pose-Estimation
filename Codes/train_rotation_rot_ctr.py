...
cemodel.load_state_dict(torch.load(os.path.join(BASE_DIR, 'models', 'center_pt', '200')))
...
model_save_dir = os.path.join(BASE_DIR, "models", str(GPU_INDEX)+"_rot_ctr_"+datetime.datetime.now().strftime('%d_%H:%M'))
...
optimizer_center = torch.optim.Adam(cemodel.parameters(), lr = 0.001)
...
loss_ctr_fn = torch.nn.L1Loss(reduce=True, size_average=True)
##################
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        ceout = cemodel(data.pos, data.batch).to(device)
        ...
        out = torch.cat((ceout, pred_angle), 1)
        ...
        loss_ctr = loss_ctr_fn(ceout, center)
        ...
        print("epoch: "+str(epoch) + "   loss: "+str(loss.data) + "     loss_cls:"+ str(loss_cls.data)+"    loss_ctr:" + str(loss_ctr.data))
        global_step += 1
        train_loss += loss.data.item() * data.num_graphs
        train_loss_cls += loss_cls.data.item() * data.num_graphs
        train_loss_ctr += loss_ctr.data.item() * data.num_graphs
    ...
    train_losses_ctr.append('{:.6f}'.format(train_loss_ctr/(len(train_dataset))))
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            eval_loss_ctr += loss_ctr.data.item() * data.num_graphs
        ...
        eval_losses_ctr.append('{:.6f}'.format(eval_loss_ctr / (len(test_dataset))))
        ...
        print('Test Loss ctr: {:.6f}'.format(eval_loss_ctr / (len(test_dataset))))
...
filename= str(GPU_INDEX)+ "_rot_ctr_"+datetime.datetime.now().strftime('%d_%H:%M')+".txt"

def writeToFileOne(epoch):
    fl = open(os.path.join(log_save_dir, filename),'a+')
    if epoch == 0:
        fl.write(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+"\n")
        fl.write("train_loss\t train_loss_cls\t train_loss_ctr\t eval_loss\t eval_loss_cls\t eval_losses_ctr\t psi\t theta \t phi \t total \n")
        return
    # fl.write("epoch: " + str(epoch)+"\n")
    for i in range(len(train_losses)):
        fl.write(train_losses[i]+ "\t" + train_losses_cls[i]+ "\t" + train_losses_ctr[i] + "\t" + eval_losses[i]+ "\t" + eval_losses_cls[i]+ "\t" + eval_losses_ctr[i] + "\t" + psi_acc[i]+ "\t" + theta_acc[i]+ "\t" + phi_acc[i]+ "\t" + total_angle_acc[i] + "\n")
    train_losses.clear()
    train_losses_cls.clear()
    eval_losses.clear()
    eval_losses_cls.clear()
    eval_losses_ctr.clear()
    psi_acc.clear()
    theta_acc.clear()
    phi_acc.clear()
...
if __name__ == '__main__':
    ...
    for epoch in range(1, NUM_EPOCHS+1):
        ...
        if epoch % EPOCHS_TO_WRITE == 0:
            ...
            torch.save(cemodel.state_dict(), os.path.join(model_save_dir, "cept_"+str(epoch)))
